...
uses com.guidewire.apd.model.generate.CCProductGenerator
%>
...
<PCF
  ...
  <Popup
    ...
    .getterCode()%>">
    <LocationEntryPoint
      signature="${info.PrefixedCode}RUPopup(aClaim : Claim)"/>
    <LocationEntryPoint
      signature="${info.PrefixedCode}RUPopup(aClaim : Claim, a${info.Code}RUParam : ${info.PrefixedCode}RU, startEditable : Boolean)"/>
    <Variable
      name="aClaim"
      type="Claim"/>
    <Variable
      ...
      type="Boolean"/>
    <Variable
      initialValue="a${info.Code}RUParam == null ? aClaim.Policy.create${info.PrefixedCode}RU() : a${info.Code}RUParam"
      name="a${info.Code}RU"
