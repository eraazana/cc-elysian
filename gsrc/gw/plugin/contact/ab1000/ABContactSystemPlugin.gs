...
uses gw.contactmapper.ab1000.ContactIntegrationMapper
...
uses wsi.remote.gw.webservice.ab.ab1000.abcontactapi.faults.PermissionException
...
uses wsi.remote.gw.webservice.ab.ab1000.abcontactapi.types.complex.RelatedContactInfoContainer
...
class ABContactSystemPlugin implements ContactSystemPlugin {
  ...
  override function createContact(contact : Contact, transactionId:String) : ContactCreateResult {
    return handleErrors(null, contact, \->{
      ...
      var visitor = new gw.plugin.contact.ab1000.TemporaryIDContactVisitor()
      traverser.traverse({visitor})

      try {
        var abContactXML = _mapper.populateXMLFromContact(contact)
        if(LOGGER.TraceEnabled) {
          LOGGER.trace("Sending XML create for: ${contact.PublicID}")
        }
  ...
  override function findDuplicates(contact : Contact, searchFilter:ContactSearchFilter) : DuplicateSearchResult {
    return handleErrors(null, contact, \ -> {
      ...
      LOGGER.trace("findDuplicates called for ${contact.PublicID}")
        ...
        switch(eventName) {
          case CONTACT_ADDED_EVENT_NAME:
              if(ContactSystemApprovalUtil.shouldCreateInContactSystem(contact)) {
                ...
                var validationResult : wsi.remote.gw.webservice.ab.ab1000.abcontactapi.types.complex.ABContactAPIValidateCreateContactResult
                try {
        ...
        if(!linkStatus.Broken) {
          ...
          switch(eventName) {
            case CONTACT_TAG_UPDATED_EVENT_NAME:
                LOGGER.trace("Creating tagupdate for Contact: ${contact.PublicID}")
            ...
            case CLAIM_CONTACT_REMOVED_EVENT_NAME:
                if (linkStatus.PendingCreate) {
                  LOGGER.trace("Creating remove for Contact: ${contact.PublicID}")
          ...
          LOGGER.trace("Broken link to CM for contact ${contact.PublicID} with abuid: ${contact.AddressBookUID}")
          payload = null
        }
      }
      if(LOGGER.TraceEnabled) {
        LOGGER.info("Event: ${eventName} for contact: ${contact.PublicID}")
      }
      if(payload!=null) {
  ...
  */
    ...
    handleErrors(null, contact, \ -> {
      ...
      if(actionType typeis CreateAction) {
        ...
        if(LOGGER.isTraceEnabled()) {
          LOGGER.trace("UpdateAction for contact: ${contact.PublicID}")
        }
        ...
        if(actionType.TagUpdateXml.length > 0 and isPending) {
          ...
          if(xml.arrayByName("Tags").XmlBackedInstance.Count != 0) {
            ...
            if(LOGGER.isTraceEnabled()) {
              LOGGER.trace("UpdateAction - TagUpdateAction for contact: ${contact.PublicID}")
            }
        ...
        xml = stripAddFromTagAdditionsWithLinkIDSet(xml)
        if(LOGGER.isTraceEnabled()) {
          LOGGER.trace("TagUpdateAction for contact: ${contact.PublicID}")
        }
        if(xml.arrayByName("Tags").XmlBackedInstance.Count != 0) {
        ...
        var abXML = wsi.remote.gw.webservice.ab.ab1000.beanmodel.XmlBackedInstance.parse(contactXML)
   ...
   */
  private function removeContact(xml: wsi.remote.gw.webservice.ab.ab1000.beanmodel.XmlBackedInstance, id : String) {
   ...
   */
  private function getContactFromRemote(remoteXmlInstance:wsi.remote.gw.webservice.ab.ab1000.beanmodel.XmlBackedInstance, contact : Contact, bundle : Bundle) : Contact {
    if(contact == null) {
   ...
   */
  private function retrieveReplacementContact(addressBookId : String) : wsi.remote.gw.webservice.ab.ab1000.beanmodel.XmlBackedInstance {
    // check to see if this contact has been replaced with another one
    var remoteXmlBackedInstance : wsi.remote.gw.webservice.ab.ab1000.beanmodel.XmlBackedInstance
    var newABUID : String
    var api = ContactAPI
  ...
  private function _retrieveContact(abUID:String, contact : Contact, bundle : Bundle) : Contact {
    var remoteXmlInstance : wsi.remote.gw.webservice.ab.ab1000.beanmodel.XmlBackedInstance
  ...
  private function internalRetrieveRelatedContacts(contact:Contact, relationships:Collection<ContactBidiRel>) : Contact {
    if(contact.AddressBookUID == null) {
      throw new IllegalArgumentException("Cannot retrieve related contacts for ${contact.PublicID} because this contact is not linked to AddressBook")
    }
  ...
  private function updateContactXml(contact:Contact,
    ...
    if(isTagUpdate or !isPending) {
      ...
      if(LOGGER.TraceEnabled) {
        LOGGER.trace("Sending XML update for contact: ${contact.PublicID}")
      }
